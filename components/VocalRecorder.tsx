  ...
  return (
    <div className="flex flex-col h-full p-3 gap-3">
      ...
      <div className="flex flex-col items-center gap-4 pt-4">
        {countdown !== null && (
          <div className="text-6xl font-bold text-[var(--accent)] animate-pulse">
            {countdown}
          </div>
        )}
        {isRecording && countdown === null && (
          <div className="text-2xl font-bold text-[var(--red)] tabular-nums">
            ● {formatTime(elapsed)}
          </div>
        )}
        <button
          ...
          className={`w-24 h-24 rounded-full flex items-center justify-center text-3xl transition-all active:scale-95 ${
            isRecording
              ? "bg-[var(--red)] shadow-[0_0_30px_var(--red)]"
              : "bg-[var(--surface2)] border-4 border-[var(--red)] text-[var(--red)]"
          }`}
        >
          {isRecording ? "■" : "●"}
        </button>
            ...
            <button
              ...
              className={`w-7 h-7 rounded text-xs border transition-colors ${
                countIn === n
                  ...
                  : "bg-[var(--surface2)] border-[var(--border)] text-gray-400"
              }`}
            >
              {n}
            </button>
      ...
      <div className="flex-1 overflow-y-auto flex flex-col gap-2">
        {takes.length === 0 && (
          <p className="text-xs text-gray-600 text-center mt-6">
            No takes yet — hit record to start
          </p>
        )}
        {takes.map((take, i) => (
          <div
          ...
          >
            <button
              onClick={() => playingId === take.id ? stopPlay(take.id) : playTake(take)}
            ...
            <button
              ...
              className="text-[10px] text-gray-500 px-2 py-1 rounded border border-[var(--border)]"
            >
              EXPORT
            </button>
            <button
